...
export const HomePage = () => {
    ...
    const fetchDataForCA = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/SupplierPortal/searchForCA`,
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    const fetchDataForDeviation = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/SupplierPortal/searchForDeviation`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  //     try {
  //       const response = await fetch("${API_BASE_URL}SupplierPortal/search", {
  //         method: 'POST',
  //         headers: {
  //           'Content-Type': 'application/json',
  //         },
  ...
  //                 `${API_BASE_URL}SupplierPortal/getAssignedPartsVisibility?suppliername=${supplierName}&partid=${partId}`
  //             );
    ...
    const fetchDataForParts = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const newVisibleSpec = [];
          const newVisibleItem = [];

          // Process all results regardless of 'isVisible'
          data.results.forEach((result) => {
            const partKey = "type: Part";
            const partInfo = result[partKey];

            if (partInfo) {
              // Collect partId, itemVisible, and specVisible values
              newVisiblePartIds.push(partInfo.partid);
              newVisibleSpec.push(partInfo.specVisible);
              newVisibleItem.push(partInfo.itemVisible);
            }
          });

          // Set the state with the collected values
          setVisiblePartIds((prev) => [...prev, newVisiblePartIds]);
          setvisibleSpec((prev) => [...prev, newVisibleSpec]);
          setVisibleItem((prev) => [...prev, newVisibleItem]);
        } else {
          console.error("Error fetching data:", response.status);
        }
      } catch (error) {
  ...
  return (
    <div>
      ...
      >
        ...
        {deviationOpen && <Devaition_Page devObjectIds={devObjectIds} />}
